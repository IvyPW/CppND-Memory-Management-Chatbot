                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //incoming - non owning references
                            (*childNode)->AddEdgeToParentNode(edge.get());
                            //(*parentNode)->AddEdgeToChildNode(edge.get());
                            //outgoing - exclusive ownership of edge
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                ...
                /*No need to dereference the pointer first with the star * operator.
Please make a habit to use the arrow operators directly with pointers.*/
                rootNode = it->get();
            }
            else
            {
